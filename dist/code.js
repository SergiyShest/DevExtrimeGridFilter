...
$(function () {
    ...
    $("#grid").dxDataGrid({
        ...
        }, { caption: "дата ", dataField: "OrderDate", dataType: "date" }, { dataField: "RequiredDate", dataType: "date" }, { dataField: "ShippedDate", dataType: "date" }, {
            dataField: "ShipVia",
    ...
    function masterDetailTemplate(container, options) {
        $("<div>").addClass("grid").appendTo(container).dxDataGrid({
            ...
            onEditorPreparing: function onEditorPreparing(e) {
                if (e.dataField === "ProductID") {
                    ...
                    e.editorOptions.onValueChanged = function (args) {
                        ...
                        var product = args.component.getDataSource().items().filter(function (data) {
                            return data.ProductID === args.value;
                        })[0];
            ...
            summary: {
                totalItems: [{ column: "Total", summaryType: "sum", displayFormat: "Total: {0}", valueFormat: { type: "currency", precision: 2 } }]
            },
            columns: [{
                dataField: "ProductID",
                caption: "Product",
            ...
            }, { dataField: "UnitPrice", format: { type: "currency", precision: 2 }, allowEditing: false }, "Quantity", "Discount", { dataField: "Total", format: { type: "currency", precision: 2 }, allowEditing: false, calculateCellValue: function calculateCellValue(data) {
                    return data.UnitPrice ? data.UnitPrice * data.Quantity * (1 - data.Discount) : null;
                } }]
        });
    }
});